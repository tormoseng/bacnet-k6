...
export function getObjectList(deviceId: number) {
  ...
  const res = http.get(`${Conf.host}/api/objectAccess/${deviceId}/objectList`, {
  ...
  try {
    ...
    const objectTypes = json.value.arrayOfObjectIdentifier.map(
      (e) => e.objectType,
    );
    ...
    //throw exp
    console.log(`[${res.status} getObjectList]`);
  }
}
...
export function getProperty(
  ...
  try {
    ...
    } else if (json.value.enumerated) {
      value = json.value.enumerated.toString();
    }
